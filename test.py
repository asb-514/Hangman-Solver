...
class HangmanAPI(object):
    ...
    def guess(self, word):  # word input example: "_ p p _ e "
        ...
        for dict_word in current_dictionary:
            ...
            if len_word > len(dict_word):
                before = dict_word
                after = dict_word
                while len(before) != len_word:
                    before = "!" + before
                    after = after + "!"
                new_dictionary.append(before)
                new_dictionary.append(after)
            if len_word < len(dict_word):
                before = dict_word
                after = dict_word
                while len(before) != len_word:
        ...
        for dict_word in new_dictionary:
            ...
            for i in range(len_word):
                ...
                for j in range(i + 1, len_word):
                    if dict_word[j] == "!":
                        # scorenow = scorenow + (
                        #    pow(prev, 2) / (26 - len(self.guessed_letters))
                        # )
                        break
                    elif clean_word[j] == dict_word[j]:
                        scorenow = scorenow + pow(prev, 2.5)
                    prev *= inc
                prev = start
